...
- **DNS(Domain Name System)** : *인터넷에서 도메인 네임을 IP 주소로 변환하는 전세계적인 거대한 <span style="color:green">분산 데이터베이스 시스템</span>*
    - DNS는 <span style="color:blueviolet">계층적 구조로 조직되어 있으며, 여러 DNS Server들이 서로 상호 작용</span>하며 동작한다.  
<br>
...
- **Stub Resolver 동작 과정**
    ...
    <br>
    2. 도메인 네임에 대한 질의를 받은 Stub Resolver는 Resolver로 DNS Query를 전달한다.  
    <br>
...
- **Resolver** : <span style="color:green">*Stub Resolver로부터 DNS Query를 받고 처리하여 Stub Resolver에게 IP 주소를 반환하는 서버*</span>  
    - Resolver는 Stub Resolver로부터 받은 DNS Query를 Name Server로 전달하고, Name Server로부터 도메인 네임에 대한 IP 주소를 받아 Stub Resolver에게 제공한다.
...
#### Name Server
- **Name Server** : *도메인 네임과 해당하는 IP 주소를 매핑하고 관리하는 서버*
    - Name Server는 <span style="color:red">계층적 구조</span>로 조직되어 있는데, 이것은 Name Server 간의 상호 작용이 계층적이라는 의미이다.
    - Name Server는 Root DNS Server, TLD DNS Server, SLD DNS Server(= Authoritative DNS Server) 나눌 수 있다.
...
- 도메인 네임을 IP 주소로 변환시키기 위해서는 Domain Name Space의 트리 구조에 대한 정보가 필요한데, 이러한 정보를 가지고 있는 서버를 DNS Server(Name Server)라고 한다.
