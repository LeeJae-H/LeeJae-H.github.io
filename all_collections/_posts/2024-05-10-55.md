...
3. **어노테이션(@WebServlet) 방식**    
    - 서블릿 클래스 생성(@WebServlet)
        ...
        ```
    > - web.xml 방식, ServletContainerInitializer 방식과는 다르게 <span style="color:blueviolet">@WebServlet 어노테이션 방식의 경우 ServletContext 객체를 초기화하기 위해 따로 web.xml이나 ServletContextListener를 사용</span>해야 한다. 
