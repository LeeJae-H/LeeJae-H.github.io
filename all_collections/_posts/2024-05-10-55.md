---
layout: post
title: (Tomcat) - ServletContainerInitializer를 사용한 서블릿 컨테이너 설정 (+web.xml) (+ServletContext)
date: 2024-05-10
categories: [tomcat]
---
...
---
> - <span style="color:blueviolet">서블릿 컨테이너가 시작될 때</span> 서블릿 컨테이너를 설정하는 작업들 중 두 가지 주요 작업인 <span style="color:blueviolet">"서블릿 등록 및 매핑"과 "ServletContext 객체 초기화"에 대해서 몇 가지 방식의 예시</span>를 살펴보자.
...
3. **어노테이션(@WebServlet) 방식**    
    - 서블릿 클래스 생성(@WebServlet)
        ...
        ```
    > - web.xml 방식, ServletContainerInitializer 방식과는 다르게 <span style="color:red">@WebServlet 어노테이션 방식의 경우 ServletContext 객체를 초기화하기 위해 따로 web.xml이나 ServletContextListener를 사용</span>해야 한다. 
    > - web.xml 방식 또는 ServletContainerInitializer 방식은 "서블릿 등록 및 매핑"과 "ServletContext 객체 초기화" 작업 모두 수행하므로 <span style="color:red">**다른 방식을 따로 사용할 필요가 없다.**</span> 
...
3. **ServletContextListener 방식**  
    ...
    **3-2. web.xml과 함께 사용하는 방식**  
    ...
    > - 일반적으로, ServletContextListener의 contextInitialized() 메소드에서 addServlet(), addFilter() 등을 사용하지 않는다. ServletContextListener의 contextInitialized() 메소드 내에서는 주로 초기화 작업을 수행하고, <span style="color:blueviolet">서블릿 및 필터 등의 등록과 같은 작업은 ServletContainerInitializer를 사용하거나 어노테이션(@WebServlet 등)을 사용</span>하는 것이 좋다.
